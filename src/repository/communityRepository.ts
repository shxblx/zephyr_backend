...
class CommunityRepository implements CommunityRepo {
    ...
    async createCommunityMembers(communityMembersData: CommunityMembers): Promise<any> {
        const newCommunityMembers = new CommunityMemberModel(communityMembersData);
        return await newCommunityMembers.save();
    }

    async fetchCommunitiesUserNotIn(userId: string): Promise<Community[]> {
        try {
    ...
    async addMemberToCommunity(communityId: string, userId: string): Promise<any> {
        try {
    ...
    async leaveCommunity(userId: string, communityId: string): Promise<any> {
        try {
            ...
            const community = await CommunityMemberModel.findOne({ communityId: communityObjectId });
            ...
            const userRemoved = result.members.every(member => !member.userId.equals(userObjectId));
            if (!userRemoved) {
    ...
    async getCommunityMembers(communityId: string): Promise<any> {
        try {
            const communityMembers = await CommunityMemberModel.findOne({ communityId })
